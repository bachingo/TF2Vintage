...
{
    SQBlob *blob;
    if(SQ_FAILED(sq_getinstanceup(v,idx,(SQUserPointer *)&blob,(SQUserPointer)SQSTD_BLOB_TYPE_TAG,SQFalse)))
        return -1;
    *ptr = blob->GetBuf();
    return SQ_OK;
}
...
SQUserPointer sqstd_createblob(HSQUIRRELVM v, SQInteger size)
{
    ...
    if(SQ_SUCCEEDED(sq_get(v,-2))) {
        ...
        if(SQ_SUCCEEDED(sq_call(v,2,SQTrue,SQFalse))
            && SQ_SUCCEEDED(sq_getinstanceup(v,-1,(SQUserPointer *)&blob,(SQUserPointer)SQSTD_BLOB_TYPE_TAG,SQFalse))) {
            sq_remove(v,-2);
            return blob->GetBuf();
        }
    }
