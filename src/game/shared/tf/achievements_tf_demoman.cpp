...
class CAchievementTFDemoman_MeleeKillWhileJumping : public CBaseTFAchievement
{
	...
	virtual void Event_EntityKilled( CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor, IGameEvent *event ) 
	{
		...
		if ( pAttacker && ( pAttacker == C_BasePlayer::GetLocalPlayer() ) )
		{
			if ( m_bStickyJumping )
			{
				if ( event->GetInt( "weaponid" ) == TF_WEAPON_BOTTLE || event->GetInt( "weaponid" ) == TF_WEAPON_SWORD || event->GetInt( "weaponid" ) == TF_WEAPON_STICKBOMB || event->GetInt("weaponid") == TF_WEAPON_SHOVEL )
				{
					IncrementCount();
				}
			}
