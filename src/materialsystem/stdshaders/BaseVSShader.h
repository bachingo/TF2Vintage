...
public:
	...
	static void RenameShaderToShaderModel30( const char *pName, char *pOutName )
	{
		Q_strcpy( pOutName, pName );
		if ( g_pHardwareConfig->SupportsShaderModel_3_0() )
		{
			size_t iLength = Q_strlen( pOutName );
			if ( ( iLength > 5 ) && Q_stricmp( pOutName + iLength - 6, "_ps20b" ) == 0 )
			{
				strcpy( pOutName + iLength - 3, "30\0" );
			}
			else if ( ( iLength > 4 ) && (
				( Q_stricmp( pOutName + iLength - 5, "_ps20" ) == 0 ) || 
				( Q_stricmp( pOutName + iLength - 5, "_vs20" ) == 0 )
				)
			)
			{
				strcpy( pOutName + iLength - 2, "30" );
			}
		}
	}
