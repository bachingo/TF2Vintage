...
float CTFFlameManager::GetFlameDamageScale( const tf_point_t* pPoint, CTFPlayer *pTFTarget /*= NULL*/ ) const
{
	...
	bool bNoFalloff =  ( (TF2VIsContemporary(TF2V_DAY_MAJOR_PYRO)) && TF2VIsAnachronistic(289) ) ? true : false ;
	...
	if ( iFlameDmgMode >= 0 && !bNoFalloff )
	{
		...
		else
		{
			...
			float flFlameMinDamageTimeCap = ( (TF2VIsAnachronistic(TF2V_DAY_MAJOR_TOUGH_BREAK) ? 175.f : 300.f) / 2450.0f ) / 0.5f; // time it takes for us to reach tf_flame_mindamagedist, as a function of lifetime. longer life does more damage at the same distance. tf_flame_min_damage_scale_time_cap
			float flFlameMinDamageTime = (TF2VIsAnachronistic(TF2V_DAY_MAJOR_TOUGH_BREAK) ? 125.f : 150.f) / 2450.0f;                // time it takes to reach tf_flame_maxdamagedist. tf_flame_damage_scale_time_min
