...
ActionResult<CTFBot> CTFBotDefendPointBlockCapture::Update( CTFBot *me, float dt )
{
	...
	CUtlVector<CTFNavArea *> *cpAreas = TFNavMesh()->GetControlPointAreas( m_pPoint->GetPointIndex() );
	if ( cpAreas )
	{
	...
	CTFBotPathCost cost( me );
	m_PathFollower.Compute( me, m_pPoint->CollisionProp()->WorldSpaceCenter(), cost );
