...
float CTFFlameManager::GetFlameDamageScale( const tf_point_t* pPoint, CTFPlayer *pTFTarget /*= NULL*/ ) const
{
	...
	if ( TF2VIsAnachronistic( TF2V_DAY_MAJOR_PYRO ) )
		iFlameDmgMode = 1;
	
	// TF2V: Briefly, no distance falloff between Pyro Update and Jul 1 2008 (Day 289)
	bool bNoFalloff =  ( TF2VIsBetween( TF2V_DAY_MAJOR_PYRO, 289 ) ) ? true : false ;
	...
	if ( iFlameDmgMode >= 0 && !bNoFalloff )
	{
		...
		if ( iFlameDmgMode == 1 )
		{
			...
			float flFlameMaxDist = TF2VIsAnachronistic( TF2V_DAY_MAJOR_TOUGH_BREAK ) ? 125.f : 150.f; // tf_flame_maxdamagedist
			float flFlameMinDist = TF2VIsAnachronistic( TF2V_DAY_MAJOR_TOUGH_BREAK ) ? 175.f : 300.f; // tf_flame_mindamagedist
			
			float flMaxDamageDistSqr = Square(flFlameMaxDist);
			float flMinDamageDistSqr = Square(flFlameMinDist);
			
			// TF2V: Values changed on Tough Break.
			float flFlameMinScale = TF2VIsAnachronistic( TF2V_DAY_MAJOR_TOUGH_BREAK) ? 0.725f : 0.5f; // tf_flame_min_damage_scale
		...
		else
		{
			...
			float flFlameMaxDamageCap = TF2VIsAnachronistic( TF2V_DAY_MAJOR_TOUGH_BREAK ) ? 175.f : 300.f;
			float flFlameMinDamage = TF2VIsAnachronistic( TF2V_DAY_MAJOR_TOUGH_BREAK ) ? 125.f : 150.f;
			
			float flFlameMinDamageTimeCap = ( flFlameMaxDamageCap / 2450.0f ) / 0.5f; // time it takes for us to reach tf_flame_mindamagedist, as a function of lifetime. longer life does more damage at the same distance. tf_flame_min_damage_scale_time_cap
			float flFlameMinDamageTime = flFlameMinDamage / 2450.0f;                // time it takes to reach tf_flame_maxdamagedist. tf_flame_damage_scale_time_min
	...
	if ( pTFTarget )
	{
		...
		if ( TF2VIsContemporary( TF2V_DAY_SPRING_2018 ) )
