...
void CHudBossHealthMeter::Update( void )
{
	if ( g_pMonsterResource && m_pHealthBarPanel )
	{
		...
		Color barColor = g_pMonsterResource->GetBossState() == 1 ? m_BossStateColor : m_BarColor;
		Color borderColor = g_pMonsterResource->GetBossState() == 1 ? m_BossStateColor : m_BorderColor;

		m_pBorderImage->SetDrawColor( borderColor );
		m_pBarImage->SetDrawColor( barColor );
	}
}
