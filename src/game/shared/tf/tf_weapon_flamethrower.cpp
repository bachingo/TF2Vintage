...
void CTFFlameThrower::PrimaryAttack()
{
	...
	if ( pOwner->GetWaterLevel() != WL_Eyes )
	{
		...
		float flDamage = (float)iDamagePerSec * flFiringInterval;
		{
			...
			flDamage = TF2VIsAnachronistic(TF2V_DAY_SPRING_2018) ? 14.f : 13.f; // tf_flamethrower_damage_per_tick
		}
#ifdef WATERFALL_FLAMETHROWER_TEST
...
float CTFFlameThrower::GetInitialAfterburnDuration() const
{
	...
	else if ( (TF2VIsContemporary(955)) && (TF2VIsAnachronistic(976)) )
		return 6.f;
	// After May 19 2010 to Jungle Inferno: Back to 10 seconds.
	else if ( (TF2VIsContemporary(976)) && ( TF2VIsAnachronistic(TF2V_DAY_MAJOR_JUNGLE_INFERNO)) )
