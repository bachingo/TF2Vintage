...
void CViewRender::Render( vrect_t *rect )
{
    ...
    for( StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t)(eEye+1) )
	{
	    ...
	    int flags = 0;
		if( eEye == STEREO_EYE_MONO || eEye == STEREO_EYE_LEFT || ( ( g_ClientVirtualReality.ShouldRenderHUDInWorld() ) || UseVRMod() ) )
		{
	...
	if( !g_ClientVirtualReality.ShouldRenderHUDInWorld() && !UseVRMod() )
	{
		CViewSetup view2d;
		view2d.x				= rect->x;
		view2d.y				= rect->y;
