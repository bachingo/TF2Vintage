...
{
	Q_memset( m_bAllowedClasses, 0, sizeof( m_bAllowedClasses ) );
}
...
{
	if ( iBotClass <= TF_CLASS_UNDEFINED || iBotClass > TF_LAST_NORMAL_CLASS )
		return false;

	return m_bAllowedClasses[ iBotClass ];
}
...
char const *CTFBotRoster::GetTeamName( void ) const
{
	return STRING( m_teamName );
}
